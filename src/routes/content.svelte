...
<div class="home-page">
  ...
  <div class="container page">
    <button class="edit" class:hidden={editor} on:click={startEdit}>Edit</button>
...
<script>
  ...
  type DraftState = "saved" | "saving..." | "waiting to save"

  let contentDiv: HTMLElement
  let editor: Quill|undefined
  let draftState: DraftState = "saved"

  const setDraftState = throttle((newState: DraftState) => {
    draftState = newState
  }, 400)
  ...
  const commit = () => {
    if (!editor) return
    $page.publish(editor.root.innerHTML)
    editor = undefined
  }

  const cancel = () => {
    editor = undefined
    contentDiv.innerHTML = $page.body
  }
</script>
...
button.publish {
  @apply rounded bg-green-600 text-white p-4 mt-4;
...
.page {
  ...
  button.edit {
    @apply hidden bg-green-600 text-white p-4 right-0 top-0 absolute z-10;
  }

  &:hover {
    button.edit:not(.hidden) {
      @apply inline-block;
    }
  }
