...
const globalCSSWatchFiles = [
  ...
  "src/styles/base.pcss", "src/styles/components.pcss", "src/styles/utilities.pcss",
]
        ...
        const buildGlobalCSS = () => {
          ...
          try {
            ...
            builder.on("close", code => {
              if (code === 0) {
                let elapsed = parseInt(performance.now() - start, 10)
                elapsed = Math.round(elapsed / 10) / 100
                console.log(`${colors.bold().green("✔ global css")} (src/styles/global.pcss → static/global.css${sourcemap === true ? " + static/global.css.map" : ""}) ${colors.gray(`(${elapsed}s)`)}`)
              } else if (code !== null) {
